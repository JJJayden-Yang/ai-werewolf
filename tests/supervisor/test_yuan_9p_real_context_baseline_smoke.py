"""A3（Yuan）：9 人 **真实 ContextAssembler** + 真实角色策略的 baseline 压测前回归。

这是 S4 的 9 人 baseline 验收入口（W3 的 9 人版）：真实 `context.ContextAssembler`
+ 真实 `RoleStrategyMockAgent`（最简策略：hunter-pass / witch-skip / 狼避队友 / 预言家查验跳报 /
平民公投），同一个 EventStore 既当 sink 又当读端，经 Supervisor.run_game 跑 100 局 9 人。

baseline 口径：不接女巫刀口激活链（witch 永远 skip）、不做 docx 高级策略。
目标：暴露"9 人真实链路还差什么"（最可能：预言家查验耗尽 → NIGHT_SEER fallback）。
"""
...
from contracts import DeathCause, EventType, GameConfig, Phase, Role
...
class _RecordingAgent:
    ...
    async def act(self, context: dict) -> dict:
        if any(k in json.dumps(context) for k in _FORBIDDEN_CONTEXT_KEYS):
...
def _run_9p_real_context(seed: int, game_id: str):
...
def _truth_wolves(engine: GameEngine, gid: str) -> set[str]:
    players = engine.get_session(gid).truth_state.players
    return {pid for pid, p in players.items() if p.role == Role.WEREWOLF}


def test_single_9p_real_context_game_observable():
    config, engine, store, _agent = _run_9p_real_context(0, "yuan_rc_9p_single")
...
def test_100_9p_real_context_games_stable_no_fallback_wolf_safe():
    stats = {
        "completed": 0,
        "phase_stuck": 0,
        "rule_validation": 0,
    ...
    for seed in range(100):
        gid = f"yuan_rc_9p_{seed:03d}"
        config, engine, store, agent = _run_9p_real_context(seed, gid)
        ...
        if any(e.event_type == EventType.HUNTER_SHOT for e in events):
            stats["hunter_shot_games"] += 1
        if any(
            e.event_type == EventType.PHASE_STARTED and e.phase == Phase.DAY_TIE_REVOTE
            for e in events
        ):
            stats["tie_revote_games"] += 1
    ...
    assert stats["rule_validation"] == 0, stats
    assert stats["fallback"] == 0, stats
    assert stats["wolf_targeting_actions"] > 0
    assert stats["wolf_hit_teammate"] == 0
